...
import { CategoryChildrenList, getCategoryDetail, getHistoryList, getMarketingCategoryList, getWishList, MarketingCategory } from '../action/creativityAction'

export interface CreativityState {
  category: MarketingCategory[]
  history: MarketingCategory
  wish: MarketingCategory
  currentCategory: CategoryChildrenList
  pureCategory: MarketingCategory[]
}
export const initialState: CreativityState = {
  ...
  currentCategory: {} as CategoryChildrenList,
  pureCategory: [] // 原始数据
}
...
const creativitySlice = createSlice({
  ...
  reducers: {
    ...
    initCurrentCategory(state) {
      state.currentCategory = {} as CategoryChildrenList
    },
    ...
    // 更新收藏列表
    updateWishList(state, action: PayloadAction<{ category: CategoryChildrenList; is_wish: 0 | 1 }>) {
      const { category, is_wish } = action.payload

      let newList: CategoryChildrenList[]
      if (is_wish === 0) {
        // 添加到收藏列表
      ...
      const newCategoryList: MarketingCategory[] = state.category.map((categoryItem) => {
        return {
          ...categoryItem,
      ...
      const newHistoryList: CategoryChildrenList[] = state.history.list.some((historyItem) => historyItem.id === category.id) ? state.history.list.map((historyItem) => (historyItem.id === category.id ? { ...historyItem, is_wish: is_wish === 0 ? 1 : 0 } : historyItem)) : state.history.list
      state.wish.list = newList
      state.category = newCategoryList
      state.history.list = newHistoryList
    },
    // 更新最近使用记录列表
    updateHistoryList(state, action: PayloadAction<MarketingCategory>) {
      state.history = action.payload
    }
  },
  extraReducers(builder) {
    // 成功之后保存用户信息
    builder.addCase(getMarketingCategoryList.fulfilled, (state, { payload }: PayloadAction<void | MarketingCategory[]>) => {
      if (payload) {
    ...
    builder.addCase(getCategoryDetail.fulfilled, (state, { payload }: PayloadAction<void | CategoryChildrenList>) => {
      state.currentCategory = payload ?? ({} as CategoryChildrenList)
    })
    builder.addCase(getWishList.fulfilled, (state, { payload }: PayloadAction<void | CategoryChildrenList[]>) => {
      state.wish.list = payload ?? []
    })
    builder.addCase(getHistoryList.fulfilled, (state, { payload }: PayloadAction<void | CategoryChildrenList[]>) => {
