...
`.trim()
  ...
  await writeFile(path.resolve(currentPath, `${fileName}.module.css`), `${componentCSSRender({ assetPath: path.join('@/icons/assets', ...pathList.slice(2), entry) })}\n`)
...
const walk = async (entry, pathList, replaceFillOrStrokeColor) => {
  ...
  try {
    ...
    if (stat.isFile() && /.+\.svg$/g.test(entry)) await generateSvgComponent(fileHandle, entry, pathList, replaceFillOrStrokeColor)
