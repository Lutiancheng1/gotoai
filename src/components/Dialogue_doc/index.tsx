...
type Props = {
  ...
  fileIds?: string[]
  insertEditValue?: (key: string, value: string) => void
} & Partial<talkInitialState>

const Dialogue = forwardRef(({ isNewChat, conversitionDetailList, currentConversation, style, firstSend, placeholder = '输入你的问题或需求', hasUploadBtn = false, initChildren, autoToBottom = true, fileId, sse = false, hasFooter = true, multiple, fileIds, insertEditValue }: Props, ref) => {
  // 初始化问题Id
                          ...
                          {!currentUUID && !item.isLoading && (
                            <div className="flex justify-between text-[14px] mt-3">
                              ...
                              <div
                                ...
                                onClick={() => {
                                  insertEditValue && insertEditValue(activeKey, item.content)
                                }}
                              >
                                插入
                              </div>
